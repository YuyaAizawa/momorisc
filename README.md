...
画面上部のタブ（`Editor`, `Runner`, `Samples`）をクリックすることでそれぞれの機能にアクセスする．

起動直後は`Runner`タブが表示され，△ボタンでサンプルプログラムを実行できる．
...
- `Program`: ソースコード
  - `Compile`ボタン: 現在のコードをコンパイルし`Runner`に移動
- `DefaultInput`: プログラムに与える入力のデフォルト値
...
上記コマンドを実行した後，`http://localhost:8000/public/index.html`をブラウザで開いて実行する．
